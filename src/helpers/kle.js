...
const disallowedAlignmentForLabels = [
  [1, 2, 3, 5, 6, 7], // 0
  [2, 3, 6, 7], // 1
  [1, 2, 3, 5, 6, 7], // 2
  [1, 3, 5, 7], // 3
...
function reorderLabels(key, { textSize }) {
  ...
  for (let i = 0; i < 12; ++i) {
    const ndx = labelMap[ret.align].indexOf(i)
    if (ndx >= 0) {
  ...
  for (let i = 0; i < ret.textSize.length; ++i) {
    if (!ret.labels[i]) ret.textSize[i] = textSize[i]
...
function serialize(keyboard) {
  ...
  for (const metakey in keyboard.meta) {
    serializeProp(meta, metakey, keyboard.meta[metakey], DEFAULT_META[metakey])
  }
